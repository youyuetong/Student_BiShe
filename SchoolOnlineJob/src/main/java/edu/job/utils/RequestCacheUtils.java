...
public class RequestCacheUtils {
    ...
    private static Logger LOGGER = LoggerFactory.getLogger(RequestCacheUtils.class);
     ...
     * 时间一分钟
     */
    public static final Long ONE_MINUTE = 24 * 60 * 60 * 1000L;
    /**
     * 缓存对象
     */
...
class CleanTimeOutThread implements Runnable{
    ...
    @Override
    public void run() {
        RequestCacheUtils.setCleanThreadRun();
        while (true) {
            System.out.println("clean thread run ");
            RequestCacheUtils.deleteTimeOut();
            try {
                Thread.sleep(RequestCacheUtils.ONE_MINUTE);
